...
def show_reminds(user_id='%%', guild_id='%%'):
  conn = sqlite3.connect(USER_GENERATED_DB)
  c = conn.cursor()
  c.execute('SELECT * FROM remind WHERE user_id LIKE ? AND guild_id LIKE ? ORDER BY requested_time ASC', (user_id, guild_id))
  rows = c.fetchall()
  conn.close()
  return rows
...
def insert_remind(requested_time, channel_id, user_id, message, guild_id):
  conn = sqlite3.connect(USER_GENERATED_DB)
  c = conn.cursor()
  c.execute('INSERT INTO remind(requested_time, channel_id, user_id, message, guild_id) VALUES (?,?,?,?,?)', (requested_time, channel_id, user_id, message, guild_id))
  conn.commit()
  conn.close()
...
def delete_remind(requested_time, channel_id, user_id, message, guild_id):
  conn = sqlite3.connect(USER_GENERATED_DB)
  c = conn.cursor()
  c.execute('DELETE FROM remind WHERE requested_time = ? AND channel_id = ? AND user_id = ? AND message = ? AND guild_id = ?', (requested_time, channel_id, user_id, message, guild_id))
  conn.commit()
  conn.close()
